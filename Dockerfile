...
RUN conda create -n r-reticulate --override-channels -c conda-forge -y python=3.10 pyarrow numpy pandas matplotlib polars plotnine statsmodels scipy patsy
# 추가로 필요한 패키지 설치

# 5. R 패키지 설치 (reticulate 및 필수 패키지)
RUN R -e "install.packages(c('reticulate', 'remotes', 'IRkernel', 'tidyverse', 'NHANES', 'MASS', 'broom', 'Lahman'))" && \
